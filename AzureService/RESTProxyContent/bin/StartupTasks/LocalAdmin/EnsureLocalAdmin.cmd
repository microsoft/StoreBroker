...
SET startuptasklogsubdir=EnsureLocalAdmin
...
IF "%ComputeEmulatorRunning%" == "true" (
    ECHO Detected running in an emulator. Skipping admin creation. 2>&1
) ELSE (
    ECHO Starting up %0. >> "%startuptasklog%" 2>&1

    :: Call the file, redirecting all output to the StartupLog.txt log file.
    powershell -executionpolicy unrestricted ". %0\..\Ensure-LocalAdmin.ps1; Ensure-LocalAdmin -userName '%LOCAL_ADMIN_USERNAME%' -secret '%LOCAL_ADMIN_SECRET%' -certThumbprint '%LOCAL_ADMIN_CERT_THUMBPRINT%' -certStore '%LOCAL_ADMIN_CERT_STORE%' -Verbose" >> "%startuptasklog%" 2>&1
)
