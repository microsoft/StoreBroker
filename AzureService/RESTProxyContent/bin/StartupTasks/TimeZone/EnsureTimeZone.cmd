...
SET startuptasklogsubdir=TimeZone
...
IF "%ComputeEmulatorRunning%" == "true" (
	ECHO Detected running in an emulator. Skipping setting the time zone. 2>&1
) ELSE (
	ECHO Starting up %0. >> "%startuptasklog%" 2>&1

	:: Call the file, redirecting all output to the StartupLog.txt log file.
	powershell -executionpolicy unrestricted ". %0\..\Ensure-TimeZone.ps1; Ensure-TimeZone -TimeZoneId '%TimeZone%' -Verbose"  >> "%startuptasklog%" 2>&1
)
